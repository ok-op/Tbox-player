...
const ERROR_404 = {"ok":false,"error_code":404,"description":"Bad Request: missing /?file= parameter", "credit": "https://github.com/SudoR2spr"};
...
class Bot {
  ...
  static async answerInlineArticle(query_id, title, description, text, reply_markup=[], id='1') {
    const data = [{type: 'article', id: id, title: title, thumbnail_url: "https://i.ibb.co/PGRRHL3n/Op-warning.png", description: description, input_message_content: {message_text: text, parse_mode: 'markdown'}, reply_markup: {inline_keyboard: reply_markup}}];
...
async function onInline(event, inline) {
  ...
  if (!PUBLIC_BOT && inline.from.id != BOT_OWNER) {
    const buttons = [[{ text: "🫧 𝐒𝐨𝐮𝐫𝐜𝐞 𝐂𝐨𝐝𝐞 〇💦", url: "https://t.me/Opleech_WD" }]];
    return await Bot.answerInlineArticle(inline.id, "Access forbidden", "Deploy your own filestream-cloudflare-workser.", "*❌ Access forbidden.*\n📡 Deploy your own [filestream-cloudflare](https://t.me/Opleech_WD) bot.", buttons)
  }
  ...
  if (fType == "image/jpg") {
    const buttons = [[{ text: "✍ 𝐒𝐞𝐧𝐝 𝐀𝐠𝐚𝐢𝐧", switch_inline_query_current_chat: inline.query }]]
...
async function onMessage(event, message) {
  ...
  if (message.document){
    ...
    const buttons = [[{ text: "🫧 𝐒𝐨𝐮𝐫𝐜𝐞 𝐂𝐨𝐝𝐞 〇💦", url: "https://t.me/Opleech_WD" }]];
    return Bot.sendMessage(message.chat.id, message.message_id, "📤 *Dear User,*\n\nSend me any 📁 File / 🎬 Video / 🎵 Audio / 🎭 GIF, and I'll provide you with a *Stream & Download Link* instantly! 🚀\n\n*🔹 File Limit:* `<= 4GB`\n*🔹 Embedded Limit:* `<= 20MB`", buttons)
  }
  ...
  const buttons = [
    [{ text: "📢 𝐓𝐞𝐥𝐞𝐠𝐫𝐚𝐦 𝐋𝐢𝐧𝐤", url: final_tele }, { text: "𝐥𝐧𝐥𝐢𝐧𝐞 𝐋𝐢𝐧𝐤 🔗", switch_inline_query: final_hash }],
    [{ text: "🎥 𝐒𝐭𝐫𝐞𝐚𝐦 𝐋𝐢𝐧𝐤", url: final_stre }, { text: "𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐋𝐢𝐧𝐤 📥", url: final_link }]
  ];

  let final_text = `*🗂 𝐅𝐢𝐥𝐞 𝐍𝐚𝐦𝐞:* \`${fName}\`\n*⚙️ 𝐅𝐢𝐥𝐞 𝐇𝐚𝐬𝐡:* \`${final_hash}\``
